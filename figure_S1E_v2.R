...
GenomicRanges::mcols(Go3[as.numeric(col_ord$tad_idx_chr)])["tad_classes_chipdensity_4classes_GEO"] <- as.character(col_ord_fix$tad_class_fix)
...
sum_All_dt[,(signal) := lapply(.SD, function(x) as.vector(rangeMinMax(x, 0, 1))),.SDcols = colnames(sum_All_dt[,.SD,.SDcols = signal])]
